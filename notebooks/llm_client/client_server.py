...
class RouteOpenAPI(RouteBase):
    ...
    async def get_models(self, headers={}, refresh=False):
        ...
        try: 
            ...
            self.models = {m.get("id"): m for m in model_list}
            return model_list
...
class RouteVanity(RouteBase):

    model_names: List[str] = Field(description="Static Names To Recognize")
    ext_cmps: List[str] = Field(description="Acceptable substrings for has_model")

    async def get_models(self, headers={}, refresh=False):
        ## Model discovery not yet implemented for vanity URLs
        model_list = [{
            'id': name,
            'object': 'model',
            'created': 735790403,
    ...
    async def has_model(self, model_name, ext, force_refresh=False):
        ## Heuristic: These are common extension components for 
        return any((s in ext) for s in self.ext_cmps)
